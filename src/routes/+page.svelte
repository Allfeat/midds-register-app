<script lang="ts">
  ...
  let isEnded: boolean = $derived(
    appState.currentStep === AppSteps.Result && appState.resultState !== null,
  );
</script>
      ...
      {#if !isEnded}
        ...
        {:else if step === AppSteps.Sending}
          {#if appState.selectedMiddsEntity !== null && appState.walletState.accounts?.[0] !== undefined}
            <Sending
              midds={appState.selectedMiddsEntity}
              account={appState.walletState.accounts[0]}
            />
          {/if}
        {/if}
      {:else if appState.resultState !== null}
